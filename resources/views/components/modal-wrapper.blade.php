<div class="relative z-50" x-data="{ visible: @entangle('visible') }" x-show="visible" x-on:keydown.escape.window=" visible=false">
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div class="flex flex-col min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div class="relative w-full h-screen  transform overflow-hidden text-end rounded-t-2xl bg-white-500 px-4 mt-8 md:mt-16 shadow-xl transition-all overflow-y-auto" x-show="visible" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-out duration-300" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90">
                <div>
                    {{ $slot }}
                </div>
            </div>
